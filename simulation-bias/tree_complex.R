...
### set up outcome as a tree (complicated version)
for(i in 1:n) {
  if(x[i] < -2) y[i] <- rnorm(1, mean = mp[i] + 2, sd = sd)
  else if(x[i] < -1) y[i] <- rnorm(1, mean = mp[i] + 4, sd = sd)
  else if(x[i] < -0.5) y[i] <- rnorm(1, mean = mp[i] + -2, sd = sd)
  else if(x[i] < 0.25) y[i] <- rnorm(1, mean = mp[i] + 0, sd = sd)
  else if(x[i] < 0.5) y[i] <- rnorm(1, mean = mp[i] + 0.7, sd = sd)
  else if(x[i] < 1.5) y[i] <- rnorm(1, mean = mp[i] + -0.7, sd = sd)
  else y[i] <- rnorm(1, mean = mp[i] + -3.7, sd = sd)
}
...
save(ests, file = "tree_complex.Rdata")
